...
export function AppShell({ children, snapshot, refreshing, onRefresh }: ShellProps) {
  ...
  return (
    <div className="min-h-[100dvh] bg-background text-foreground">
      <aside id="mobile-navigation" className={`fixed inset-y-0 left-0 z-50 flex w-[268px] flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground transition-transform duration-200 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-20 items-center justify-between border-b border-sidebar-border px-6">
          ...
          <button type="button" className="focus-ring min-h-11 min-w-11 touch-manipulation rounded-md p-1.5 text-sidebar-foreground/55 hover:bg-sidebar-accent lg:hidden" onClick={closeMobileNav} data-testid="button-close-mobile-nav" aria-label="Close navigation"><X size={17} /></button>
        </div>
      ...
      {mobileOpen && <button type="button" className="fixed inset-0 z-40 bg-sidebar/45 lg:hidden" onClick={closeMobileNav} data-testid="button-dismiss-mobile-nav" aria-label="Dismiss navigation" />}
      <div className="lg:pl-[268px]">
        <header className="sticky top-0 z-40 flex h-[72px] items-center justify-between border-b border-border bg-background/90 px-4 backdrop-blur-md sm:px-8">
           <div className="relative z-10 flex items-center gap-3"><button type="button" className="focus-ring relative z-10 min-h-11 min-w-11 touch-manipulation select-none rounded-lg border border-border bg-card p-2 lg:hidden" onClick={toggleMobileNav} data-testid="button-open-mobile-nav" aria-label={mobileOpen ? 'Close navigation' : 'Open navigation'} aria-expanded={mobileOpen} aria-controls="mobile-navigation"><Menu size={17} /></button><div className="hidden items-center gap-2 text-xs text-muted-foreground sm:flex"><span className="font-mono-ui text-[11px]">FOUNDER OS</span><ChevronRight size={13} /><span className="font-semibold text-foreground">{navItems.find((item) => item.href === '/' ? location === '/' : location.startsWith(item.href))?.label || 'Workspace'}</span></div></div>
